...
`src/eugene_plexus_connector/_generated/`.
...
GENERATED_DIR = REPO_ROOT / "src" / "eugene_plexus_connector" / "_generated"
WORKING_DIR = REPO_ROOT / ".codegen-cache"

SPECS_TARBALL_URL_TEMPLATE = "https://github.com/eugene-plexus/specs/archive/{ref}.tar.gz"

# Three inputs:
#   - connector.yaml for our own AdapterStatus + path-scoped response shapes
#   - common.yaml for shared schemas (Health, ConfigDocument, AdapterEntry,
#     PendingIdentityLink, MessageSource, ChannelContextEntry, …)
#   - orchestrator.yaml for ChatRequest / ChatResponse — the connector
#     posts ChatRequest to /v1/chat as its outbound contract
SPECS_TO_GENERATE = [
    ("openapi/connector.yaml", "models.py"),
    ("openapi/components/common.yaml", "common_models.py"),
    ("openapi/orchestrator.yaml", "orchestrator_models.py"),
]
...
def run_codegen(specs_root: Path, ref: str) -> None:
    ...
    for input_rel, output_name in SPECS_TO_GENERATE:
        ...
        cmd = [
            ...
            "--disable-timestamp",
            # Follow paths-scope refs so inline response shapes
            # (ListAdaptersResponse-style objects defined in `paths`
            # rather than `components.schemas`) end up in the output.
            "--openapi-scopes",
            "paths",
            "schemas",
        ]
