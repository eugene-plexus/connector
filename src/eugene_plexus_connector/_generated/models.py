...
class AdapterStatus(BaseModel):
    """
    Combined declarative + operational view of one adapter.
    Mirrors the watchdog's `Component` shape pattern.

    """

    entry: AdapterEntry
    status: Status = Field(..., description="Adapter loop's current state.")
    connectedAt: AwareDatetime | None = Field(
        None,
        description='When the adapter last successfully connected to its\nplatform. Resets on each (re)connection.\n',
    )
    lastError: str | None = Field(
        None,
        description='Most recent error message, if any. Reset to null on\nsuccessful connection. Useful for surfacing why an\nadapter keeps disconnecting.\n',
    )
    platformIdentity: dict[str, Any] | None = Field(
        None,
        description="Adapter-supplied summary of the platform identity it's\nauthenticated as (e.g. Discord: bot user id + username).\nSurfaced in the UI's adapter list so the operator can\nverify they're authenticated as the right bot.\n",
    )
...
class V1AdaptersGetResponse(BaseModel):
