"""Auth state for the connector component.

Built once at startup from env vars the watchdog threads in:

  * `EUGENE_PLEXUS_CONNECTOR_AUTH_SIGNING_KEY` — base64 of the 32-byte
    HMAC key used to validate inbound bearer tokens.
  * `EUGENE_PLEXUS_CONNECTOR_SERVICE_TOKEN` — long-lived JWT
    (`aud: service:connector`). Connector uses this for OUTBOUND
    calls to orchestrator (POST /v1/chat) and identity (POST
    /v1/identity/links/pending, GET /v1/identity/persons).
  * `EUGENE_PLEXUS_CONNECTOR_MASTER_KEY` — base64 secretbox key.
    Used to seal sensitive adapter config fields (bot tokens) at
    rest, identical pattern to hemisphere-driver.
...
def load_auth_state(
    *,
    ...
    if signing_key is None:
        ...
        log.warning(
            "EUGENE_PLEXUS_CONNECTOR_AUTH_SIGNING_KEY not set — running unauthenticated "
